...
serve(async (req) => {
  ...
  const { data: tasks, error: tasksError } = await supabase
    .from('tasks')
    .select('id, firm_id, client_id, task_type, custom_name, due_date, clients(name, phone), firms(name)')
  ...
  for (const task of tasks ?? []) {
    ...
    const { text, parameters } = buildReminderMessage(client.name, firm?.name ?? 'your CA', taskLabel, task.due_date)
  ...
  return new Response(JSON.stringify({ scanned: tasks?.length ?? 0, sent, failed, skippedNoPhone }), {
