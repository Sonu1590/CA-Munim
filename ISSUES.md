...
### M15. Invoice Settings panel can never load or save real data — wrong column name (`footer_notes` vs `default_notes`)
**File:** `src/data/Settings.ts` (`fetchInvoiceSettingsFromSupabase`, `saveInvoiceSettingsToSupabase`)
Found while writing E2E coverage for Settings (`e2e/09-settings.spec.ts`). Both functions reference an `invoice_settings.footer_notes` column that does not exist — confirmed against the live DB schema, the real column is `default_notes` (`select column_name from information_schema.columns where table_name = 'invoice_settings'` returns `id, firm_id, prefix, reset_annually, default_terms, default_notes, gst_rate, signatory_name, signature_url, created_at` — no `footer_notes`, and note `reset_per_fy`/`payment_terms` used by the fetch/save code don't match `reset_annually`/`default_terms` either, so this is more than a one-column mismatch). Two separate, compounding failures:
- **Fetch silently lies.** `fetchInvoiceSettingsFromSupabase`'s `.select(...)` requests the nonexistent columns; on any error it does `if (error || !data) return mockInvoiceSettings` (the same L5-pattern silent-mock-fallback this file was already flagged for). Confirmed live: the panel always displays `mockInvoiceSettings`'s hardcoded placeholder data verbatim ("CA Rajesh Sharma", "rajesh@sharma-ca.in", "Payment due within 15 days of invoice date") regardless of which firm is signed in — a firm's real invoice numbering/GST rate/signatory would never show even if it existed. (The `invoice_settings` table is also currently empty for every firm, so the fetch fails for the additional reason of `.single()` finding 0 rows — but the column-name bug would break it even with rows present.)
- **Save fails outright, with the error surfaced to the user.** `saveInvoiceSettingsToSupabase`'s `.upsert(...)` includes `footer_notes`, so PostgREST rejects the whole write with `Could not find the 'footer_notes' column of 'invoice_settings' in the schema cache`. Nothing a user configures on this tab can ever be saved. Reproduced via the new E2E test, which asserts this exact error toast (a deliberate "document the current broken behavior" test, not a false round-trip-success test — see the comment above it in `09-settings.spec.ts`).
**Fix:** rename `footer_notes`→`default_notes`, `reset_per_fy`→`reset_annually`, `payment_terms`→`default_terms` (or migrate the DB columns to match the frontend's names, whichever is the intended contract) in both functions, and once real firm data can round-trip, replace the placeholder E2E test with a proper same-value round-trip test like the one used for Firm Profile.
