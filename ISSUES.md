...
- **iOS/Android app.** User wants this app available as native (or near-native) mobile apps, not just responsive web (2026-07-13 request). Not scoped or started — needs a technology decision first (e.g. Capacitor wrapping the existing React/Vite app vs. React Native rewrite vs. PWA-only) before any implementation work, given the size of the decision and its downstream effect on the whole codebase.
