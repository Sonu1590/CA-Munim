...
Genuinely still open: INC-20A/PAS-3 are one-time, event-based filings (relative to an incorporation/allotment date) that don't fit the FY-relative `due_date_rule` shape at all — a larger feature (event-date capture per client) than this fits, and better scoped as its own follow-up.
